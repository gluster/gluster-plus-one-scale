...
def gluster_check_bricks_status(vol_name):
    ...
    """
    ret = True
    bricks_status = gf_get_volume_status_xml(vol_name)
...
def gluster_check_volume_heal_summary(vol_name):
    ...
    """
    ret = True
    heal_summary = gf_get_volume_heal_summary_xml(vol_name)
    if not heal_summary:
        return False
...
def gluster_volume_is_ready_to_scale(vol_name, list_of_empty_drives):
    ...
    try:
        ...
        vol_info = gf_get_volume_info_xml(vol_name)
    except GfCommandFailed as e:
        glogger.exception(e.message)
        return False

    vol_config = gf_get_volume_config(vol_info)
...
def gluster_create_brick_map_to_swap(vol_name="", new_host="",
    ...
    if not vol_name:
        vol_info = gf_get_test_volume_info(subvol=6, data=4, red=2)
        for elm in vol_info["bricks_list"]:
            print (elm)
    else:
        vol_info = gf_get_volume_info_xml(vol_name)

    vol_brick_list = gf_get_volume_bricks_list(vol_info)

    vol_config = gf_get_volume_config(vol_info)
    brcount = vol_config['disperseCount']

    subvol_brick_list = gf_subvol_host_to_bricks_dict(vol_brick_list, brcount)
