"""This script facilitates plus one scaling."""
...
def gf_get_volume_and_bricks():
    ...
    if not volname or not filename or not hostname:
        glogger.debug('Invalid volume name and filename')
...
def gf_migrate_bricks_using_swap_map(volname, bricks_migration_map):
    ...
    while True:
        try:
            ...
            glogger.debug('Invalid swap option')
...
def main():
    ...
    while True:
        try:
            ...
            break
    try:
        list_of_empty_drives = gf_get_new_bricks_list(filename)
        print ("**************New Bricks*****************")
